...
func main() {
	...
	river.AddWorker(workers, governance.NewWebhookDeliveryWorker(db.Pool))
	...
	// Anonymous audit-ready 14-day sweep. Daily cron that emails the
	// pilot-report-ready notice to prospects whose audit window has
	// closed. Without an email client (no SENDGRID_API_KEY), the
	// worker no-ops and logs a one-line note — operators decide.
	var auditEmailer email.Client
	if key := os.Getenv("SENDGRID_API_KEY"); key != "" {
		from := os.Getenv("SENDGRID_FROM_ADDRESS")
		if from == "" {
			from = "noreply@bastio.com"
		}
		fromName := os.Getenv("SENDGRID_FROM_NAME")
		if fromName == "" {
			fromName = "Bastio"
		}
		auditEmailer = email.NewSendGridClient(key, from, fromName)
	}
	river.AddWorker(workers, audit.NewReadySweepWorker(
		db.Pool, archiveCH, auditEmailer, audit.LoadConfig(),
	))
	...
	client, err := river.NewClient(riverpgxv5.New(db.Pool), &river.Config{
		...
		PeriodicJobs: []*river.PeriodicJob{
			...
			// Daily audit-ready sweep — emails prospects whose 14-day
			// pending audit just closed with the activation URL. Closes
			// the wedge funnel; without it, prospects would have to
			// remember the activation URL on their own. Same dedupe
			// semantics as the archive sweep above.
			river.NewPeriodicJob(
				river.PeriodicInterval(24*time.Hour),
				func() (river.JobArgs, *river.InsertOpts) {
					return audit.ReadySweepArgs{}, nil
				},
				&river.PeriodicJobOpts{RunOnStart: true},
			),
		},
