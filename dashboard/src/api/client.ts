...
export const api = {
  ...
  governance: {
    overview: (params?: { window_days?: number }) =>
      unwrap(
        http.GET("/v1/governance/dashboard/overview", {
          params: { query: params ?? {} },
        }),
      ),

    events: (params?: { limit?: number; severity?: "low" | "medium" | "high"; source_domain?: string }) =>
      unwrap(
        http.GET("/v1/governance/dashboard/events", {
          params: { query: params ?? {} },
        }),
      ),

    overrides: (params?: { limit?: number }) =>
      unwrap(
        http.GET("/v1/governance/dashboard/overrides", {
          params: { query: params ?? {} },
        }),
      ),

    deployments: () =>
      unwrap(http.GET("/v1/governance/dashboard/deployments", {})),

    installations: {
      list: () =>
        unwrap(http.GET("/v1/governance/dashboard/installations", {})),
      create: (label: string) =>
        unwrap(
          http.POST("/v1/governance/dashboard/installations", {
            body: { label },
          }),
        ),
      revoke: (id: string) =>
        unwrap(
          http.DELETE("/v1/governance/dashboard/installations/{id}", {
            params: { path: { id } },
          }),
        ),
      // MDM bundle download URL (binary; not type-safe through typed client)
      bundleURL: (id: string) =>
        `/api/v1/governance/dashboard/installations/${id}/mdm-bundle.zip`,
    },

    policy: {
      get: () => unwrap(http.GET("/v1/governance/dashboard/policy", {})),
      update: (data: GovernanceCustomerPolicy) =>
        unwrap(
          http.PUT("/v1/governance/dashboard/policy", { body: data }),
        ),
    },

    webhooks: {
      list: () => unwrap(http.GET("/v1/governance/dashboard/webhooks", {})),
      create: (data: GovernanceCreateWebhookRequest) =>
        unwrap(
          http.POST("/v1/governance/dashboard/webhooks", { body: data }),
        ),
      delete: (id: string) =>
        unwrap(
          http.DELETE("/v1/governance/dashboard/webhooks/{id}", {
            params: { path: { id } },
          }),
        ),
      test: (id: string) =>
        unwrap(
          http.POST("/v1/governance/dashboard/webhooks/{id}/test", {
            params: { path: { id } },
          }),
        ),
    },
    ...
    domains: {
      list: () =>
        unwrap(http.GET("/v1/governance/dashboard/domain-overrides", {})),
      add: (data: GovernanceAddDomainRequest) =>
        unwrap(
          http.POST("/v1/governance/dashboard/domain-overrides", {
            body: data,
          }),
        ),
      delete: (id: string) =>
        unwrap(
          http.DELETE("/v1/governance/dashboard/domain-overrides/{id}", {
            params: { path: { id } },
          }),
        ),
    },
    ...
    pilotReport: () =>
      unwrap(http.GET("/v1/governance/dashboard/pilot-report", {})),

    pilotReportPDFURL: () => `/api/v1/governance/dashboard/pilot-report.pdf`,
  },
};
