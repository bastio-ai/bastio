...
// The /v1/workspace/audit READ endpoint (listAudit) is cloud-only — it
// surfaces multi-actor history that doesn't apply to OSS single-tenant
// single-user installs. The handler now lives in
// bastio-cloud/internal/workspace. This file keeps the WRITE primitives
// (audit(), auditCrossUserRead(), AuditEntry, AuditTarget, the
// lookupMemberEmail helper) — all called by OSS-legitimate handlers
// (assistants, knowledge, settings, conversations) and shared with
// cloud's handlers via the exported workspace.Store.WriteAudit method.
