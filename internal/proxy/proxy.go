...
// ResolveTenantKey returns any provider key the customer has stored
// for `provider`, preferring an explicitly-marked default and falling
// back to a per-proxy key when no default exists. Workspace chat (OSS
// single-tenant) uses this — it has no proxy id of its own and just
// needs the dashboard-managed key for the chosen provider. Errors when
// the customer has no key for the provider; callers should treat that
// as "fall back to env-var defaults".
func (s *Service) ResolveTenantKey(ctx context.Context, customerID uuid.UUID, provider providers.Provider) (string, error) {
	var encryptedKey json.RawMessage
	err := s.db.QueryRow(ctx, `
		SELECT encrypted_key FROM proxy_provider_keys
		WHERE customer_id = $1 AND provider = $2
		ORDER BY is_default DESC, proxy_id NULLS LAST
		LIMIT 1
	`, customerID, string(provider)).Scan(&encryptedKey)
	if err != nil {
		return "", fmt.Errorf("no provider key configured for %s: %w", provider, err)
	}

	// Legacy plaintext envelope (early OSS installs may have these).
	var legacy struct {
		Key string `json:"key"`
	}
	if jerr := json.Unmarshal(encryptedKey, &legacy); jerr == nil && legacy.Key != "" {
